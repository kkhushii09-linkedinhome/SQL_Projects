...
CREATE TABLE tbl_library_branch (
	library_branch_BranchID INT PRIMARY KEY IDENTITY (1,1) NOT NULL,
	library_branch_BranchName VARCHAR(100) NOT NULL,
	library_branch_BranchAddress VARCHAR(200) NOT NULL
);

CREATE TABLE tbl_borrower (
	borrower_CardNo INT PRIMARY KEY IDENTITY (100,1) NOT NULL,
...
CREATE TABLE tbl_book_loans (
	book_loans_LoansID INT PRIMARY KEY IDENTITY (1,1) NOT NULL,
...
CREATE TABLE tbl_book_copies (
	book_copies_CopiesID INT PRIMARY KEY IDENTITY (1,1) NOT NULL,
...
CREATE TABLE tbl_book_authors (
	book_authors_AuthorID INT PRIMARY KEY IDENTITY (1,1) NOT NULL,
...
-- ( ✅ Keep your original INSERT data here – it works! ) 
-- No syntax errors in INSERTs
...
-- #1
GO
CREATE OR ALTER PROC dbo.bookCopiesAtAllSharpstown 
...
BEGIN
    ...
    JOIN tbl_book book ON copies.book_copies_BookID = book.book_BookID
...
GO
CREATE OR ALTER PROC dbo.bookCopiesAtAllBranches
	@bookTitle varchar(70) = 'The Lost Tribe'
AS
BEGIN
	SELECT branch.library_branch_BranchName, copies.book_copies_No_Of_Copies
	FROM tbl_book_copies copies
	INNER JOIN tbl_book book ON copies.book_copies_BookID = book.book_BookID
	INNER JOIN tbl_library_branch branch ON copies.book_copies_BranchID = branch.library_branch_BranchID
	WHERE book.book_Title = @bookTitle
END
GO
...
GO
CREATE OR ALTER PROC dbo.NoLoans
AS
BEGIN
	SELECT borrower_BorrowerName
	FROM tbl_borrower b
	WHERE NOT EXISTS (SELECT 1 FROM tbl_book_loans bl WHERE bl.book_loans_CardNo = b.borrower_CardNo)
END
GO
